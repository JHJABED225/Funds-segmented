...
function renderHome() {
  ...
  : '—';
    return `<tr><td><div class="member-name"><div class="avatar">${getAvatar(p.memberName)}</div><span style="color:var(--muted);font-size:11px;margin-right:4px">${p.memberId||''}</span>${p.memberName}</div></td>
...
function renderGrowthTable() {
  document.getElementById('growth-table').innerHTML = state.members.map((m,i)=>{
    ...
    return `<tr>
      <td><div class="member-name"><div class="avatar">${getAvatar(m.name)}</div><div><div style="font-weight:600;font-size:13px"><span style="color:var(--muted);font-size:11px;margin-right:4px">${m.id}</span>${m.name}</div><div style="font-size:10px;color:var(--muted)">${m.role==='core'?m.position:'Member'}</div></div></div></td>
...
function renderPayments() {
  ...
  : '—'; 
    ...
    <td><div class="member-name"><div class="avatar">${getAvatar(p.memberName)}</div><span style="color:var(--muted);font-size:11px;margin-right:4px">${p.memberId||''}</span>${p.memberName}</div></td>
    <td class="mono">${C(Number(p.amount))}</td>
