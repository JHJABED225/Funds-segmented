...
function seedData() {
  const coreNames = [
    {name:'Rahim Uddin',email:'rahim@gmail.com',position:'President',role:'core'},
    {name:'Karim Hossain',email:'karim@gmail.com',position:'Vice President',role:'core'},
    {name:'Nadia Islam',email:'nadia@gmail.com',position:'Treasurer',role:'core'},
    {name:'Farhan Ahmed',email:'farhan@gmail.com',position:'Secretary',role:'core'},
    {name:'Tania Begum',email:'tania@gmail.com',position:'Joint Secretary',role:'core'},
    {name:'Shakib Khan',email:'shakib@gmail.com',position:'Finance Officer',role:'core'},
    {name:'Mitu Akter',email:'mitu@gmail.com',position:'Auditor',role:'core'}
  ];
  const memberNames = [
    'Arif Hasan','Bella Roy','Chandan Das','Dilruba Khatun','Emon Chowdhury',
    'Fatema Noor','Golam Rabbani','Hena Parvin','Imran Ali','Jhumur Biswas',
    'Kabir Uddin','Laila Hossain','Mamun Rashid','Nasrin Akter','Omar Faruq',
    'Puja Sarker','Quamrul Islam','Ripa Khanam','Sumon Barua','Topu Mondal',
    'Urmi Dey','Vashkar Paul','Warda Sultana','Xintu Mia'
  ];
  state.members = coreNames.map((m,i) => ({
    id: 'M'+(i+1).toString().padStart(3,'0'),
    name: m.name, email: m.email, role: m.role,
    position: m.position, phone:'', joinDate:'2024-01', active:true
  }));
  memberNames.forEach((name,i) => {
    state.members.push({
      id:'M'+(i+8).toString().padStart(3,'0'),
      name, email: name.split(' ')[0].toLowerCase()+'@gmail.com',
      role:'member', position:'', phone:'', joinDate:'2024-01', active:true
    });
  });
  // Seed some payments
  const now = new Date();
  state.members.forEach((m,mi) => {
    for(let mo=0; mo<6; mo++) {
      if(Math.random() > 0.15) {
        const d = new Date(2024, mo+4, 5 + Math.floor(Math.random()*10));
        state.payments.push({
          id:'P'+Date.now()+mi+mo,
          memberId:m.id, memberName:m.name,
          amount:state.settings.monthlyRate,
          type:'deposit', forMonth:mo+4, forYear:2024,
  ...
  state.expenses = [
    {id:'E001',title:'Stationery',category:'Administrative',amount:250,date:'2024-05-02',by:'Nadia Islam',note:'Pens and files'},
    {id:'E002',title:'Eid Celebration',category:'Event',amount:2500,date:'2024-04-10',by:'Farhan Ahmed',note:''},
    {id:'E003',title:'Meeting Snacks',category:'Administrative',amount:800,date:'2024-06-15',by:'Nadia Islam',note:'Monthly meeting'},
    {id:'E004',title:'Emergency Fund',category:'Emergency',amount:3000,date:'2024-07-20',by:'Rahim Uddin',note:'Member assistance'}
  ];
  saveState();
}
...
function navigate(page) {
  ...
  document.querySelectorAll('.nav-item').forEach(n => {
    if(n.textContent.trim().toLowerCase().includes(page === 'home' ? 'home' :
       ...
       page === 'expenses' ? 'expense' : page === 'members' ? 'member' : page === 'my-details' ? 'my details' : 'setting')) {
...
function renderHome() {
  ...
  }).join('') || '<tr><td colspan="5" style="text-align:center;color:var(--muted);padding:24px">No payments yet</td></tr>';
}
...
function renderPayments() {
  ...
  }).join('') || '<tr><td colspan="8" style="text-align:center;color:var(--muted);padding:24px">No payments found</td></tr>';
}
